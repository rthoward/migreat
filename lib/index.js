...
module.exports = {
    ...
    gen: function(settings) {
        ...
        const migrationFileBase = `${migrationTimeStamp}${labelSuffix}.js`
        const migrationFilePath = path.join(settings.migrationDir, migrationFileBase + '.js');

        const migrationFile = template(migrationFileBase)
        fs.writeFileSync(migrationFilePath, migrationFile);
